...
public class ProtectionServiceImpl extends BaseService implements ProtectionService {
        ...
        .thenCompose(units -> {
          if (unitIds.size() == units.size()) {
            ...
            if (!activeUnits.isEmpty() && applyMergingStrategy(activeUnits, operations)) {
              return verifyUserIsMemberOfOrganizationUnits(extractUnitIds(activeUnits), requestContext.getHeaders().get(OKAPI_USERID_HEADER), lang, requestContext);
            }
  ...
  private CompletableFuture<List<AcquisitionsUnit>> getUnitsByIds(List<String> unitIds, String lang, RequestContext requestContext) {
    ...
    return acquisitionsUnitsService.getAcquisitionsUnits(query, 0, Integer.MAX_VALUE, lang, requestContext)
  ...
  private CompletableFuture<Void> verifyUserIsMemberOfOrganizationUnits(List<String> unitIdsAssignedToOrg, String currentUserId,
    ...
    return acquisitionsUnitsService.getAcquisitionsUnitsMemberships(query, 0, Integer.MAX_VALUE, lang, requestContext)
      .thenAccept(unit -> {
