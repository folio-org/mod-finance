...
public class RestClient {
  ...
  private static <T> Future<HttpResponse<T>> convertHttpResponse(HttpResponse<T> response) {
    ...
    return Future.failedFuture(isJsonOfType(errorResponse, Errors.class)
      ? new HttpException(response.statusCode(), mapToErrors(errorResponse))
      : getErrorByCode(errorResponse)
